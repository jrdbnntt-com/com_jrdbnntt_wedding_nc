...
def schedule_send_rsvp_updated_email(reservation_id: int):
    task_name = 'send_rsvp_updated_email_%d' % reservation_id
    if not Task.objects.get_task(task_name=task_name).exists():
        _task_send_rsvp_updated_email(task_name=task_name, reservation_id=reservation_id)
